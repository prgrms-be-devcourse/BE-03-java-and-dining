...
public class CustomReservationRepositoryImpl extends QuerydslRepositorySupport
	...
	public CustomerReservationInfoProj findCustomerReservationInfo(
		...
		return query.select(Projections.fields(CustomerReservationInfoProj.class,
				ExpressionUtils.as(
			...
			.distinct()
			.from(reservation)
			.where(reservation.customer
				.eq(selectReservationCustomer(customerReservationInfoParam, reservation))
			)
			.fetchOne();
	}
	...
	private JPQLQuery<Member> selectReservationCustomer(
		...
		return JPAExpressions.select(reservation.customer)
			.from(reservation)
			.where(reservation.id.eq(customerReservationInfoParam.id()));
	}
